...
#@timeit to "baddodes" addodes!(rn; sparse_jac=true, build_symfuncs=false, build_paramjac=false)
...
@timeit to "CVODE_BDF-2" begin sol = solve(oprob,CVODE_BDF(),dense=false, abstol=1e-8, reltol=1e-8); end; show(to)
@timeit to "KenCarp4-1" begin sol = solve(oprob,KenCarp4(autodiff=false),dense=false, calck=false, abstol=1e-8, reltol=1e-8); end; show(to)
@timeit to "KenCarp4-2" begin sol = solve(oprob,KenCarp4(autodiff=false),dense=false, calck=false, abstol=1e-8, reltol=1e-8); end; show(to)
