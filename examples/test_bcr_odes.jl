...
using DiffEqBase, Catalyst, Plots, OrdinaryDiffEq, Sundials, DataFrames, CSVFiles, LinearAlgebra,
      SparseArrays
...
@timeit to "KenCarp4-1" begin
    sol = solve(boprob, KenCarp4(; autodiff = false); abstol = 1e-8, reltol = 1e-8, saveat = 1.0)
end;
show(to);
@timeit to "KenCarp4-2" begin
    bsol = solve(boprob, KenCarp4(; autodiff = false); abstol = 1e-8, reltol = 1e-8, saveat = 1.0)
end;
show(to);
