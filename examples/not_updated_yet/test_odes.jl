...
@timeit to "f2" rn.f(du, u, p, 0.0)
if build_jac
    J = zeros(length(u), length(u))
    @timeit to "J1" rn.jac(J, u, p, 0.0)
    @timeit to "J2" rn.jac(J, u, p, 0.0)
end
show(to)
println()
...
@timeit to "CVODE_BDF-1" begin
    sol = solve(oprob, CVODE_BDF(); dense = dense, abstol = 1e-8, reltol = 1e-8)
end;
show(to);
...
@timeit to "KenCarp4-1" begin
    sol = solve(oprob, KenCarp4(; autodiff = false); dense = dense, calck = calck, abstol = 1e-8,
                reltol = 1e-8)
end;
show(to);
