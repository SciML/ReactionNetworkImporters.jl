...
u = copy(u0);
du = similar(u);
@timeit to "f1" rnbng.f(du, u, p, 0.0)
@timeit to "f2" rnbng.f(du, u, p, 0.0)
...
function solve_dense(boprob, u, p, build_jac)
    ...
    @timeit to "KenCarp4-LU-1" begin
        bsol = solve(boprob, KenCarp4(; autodiff = false); abstol = 1e-8, reltol = 1e-8,
                     saveat = 1.0)
    end
    show(to)
...
function solve_sparse(boprob, u, p)
    ...
    @timeit to "KenCarp4-SLU-1" begin
        bsol = solve(boprob, KenCarp4(; autodiff = false, linsolve = LinSolveFactorize(lu));
                     saveat = 1.0, abstol = 1e-8, reltol = 1e-8)
    end
    show(to)
