...
const MAX_SYM_LEN = 8
make_shortsym(name, sidx) = length(name) < MAX_SYM_LEN ? Symbol(stripchars(name)) :
                            Symbol(string("S$sidx"))
...
function parse_reactions!(ft::BNGNetwork, specs, ps, t, lines, idx, idstosyms, opmod)
    ...
    while lines[idx] != REACTIONS_BLOCK_END
        ...
        empty!(cntdict)
        foreach(rid -> (rid > 0) &&
                    (haskey(cntdict, rid) ? (cntdict[rid] += 1) : (cntdict[rid] = 1)), reactantids)
        ...
        empty!(cntdict)
        foreach(pid -> (pid > 0) &&
                    (haskey(cntdict, pid) ? (cntdict[pid] += 1) : (cntdict[pid] = 1)), productids)
...
function loadrxnetwork(ft::BNGNetwork, rxfilename; name = gensym(:ReactionSystem), kwargs...)
    ...
    obseqs, groupstosyms, idx = parse_groups(ft, lines, idx, shortsymstosyms, idstoshortsyms, specs,
                                             t)
    println("done")
    ...
    # build the model    
    rn = ReactionSystem(rxs, t, specs, ps; name = name, observed = obseqs, defaults = defmap,
                        kwargs...)
    ...
    return ParsedReactionNetwork(rn; u₀ = u0map, p = pmap, varstonames = shortsymstosyms,
